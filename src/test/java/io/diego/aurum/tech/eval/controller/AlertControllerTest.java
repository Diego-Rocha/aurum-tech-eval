...
public class AlertControllerTest {
    ...
    private void initMocks() throws IOException {
        ...
        when(alertMockRepository.findAll(pageRequestCustom)).thenReturn(new PageImpl<>(alerts.subList(0, 1), pageRequestCustom, mockSize));

        PageRequest pageRequestOutOfRange = PageRequest.of(99, 1);
        when(alertMockRepository.findAll(pageRequestOutOfRange)).thenReturn(new PageImpl<>(Collections.emptyList(), pageRequestOutOfRange, mockSize));
    ...
    public void whenGetPaginate_withCustomSizeAndPage_thenOk() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/alert")
                ...
                .andExpect(content().string(containsString("\"publication\"")))
                .andExpect(content().string(containsString("\"totalElements\":" + mockSize)))
                .andDo(document("alert/list/custom"));
